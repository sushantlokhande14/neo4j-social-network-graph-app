...
/**
 * SignUpDialog component for email/password and OAuth sign-up.
...
export function SignUpDialog({
  ...
  const { signInWithOAuth, signUpWithCredential, verifyEmailCode } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await signUpWithCredential(email, password);
          ...
          <div className="flex flex-col gap-4 mt-4">
            ...
            <form onSubmit={handleVerifyCode} className="flex flex-col gap-4">
              ...
              <input
                ...
                required
                autoFocus
                className="w-full px-4 py-3 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white placeholder-white/50 text-sm focus:outline-none focus:border-purple-400/60 transition-colors text-center tracking-widest"
              />
